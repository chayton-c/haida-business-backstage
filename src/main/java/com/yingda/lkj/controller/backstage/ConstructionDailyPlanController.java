...
//        String warnStatus = req.getParameter("warnStatus"); // 风险级别
        ...
        String sql = """
                ...
                SELECT
                    ...
                    constructionControlPlan.start_time AS startTime,
                    constructionControlPlan.end_time AS endTime,
                    constructionControlPlan.start_kilometer AS startKilometer,
                    constructionControlPlan.end_kilometer AS endKilometer,
                    constructionControlPlan.add_time AS addTime,
                	leftStation.id AS startStationId,
                    leftStation.`name` AS startStationName,
                    rightStation.`name` AS endStationName,
                	railwayLine.id AS railwayLineId,
                    railwayLine.`name` AS railwayLineName,
                    constructionControlPlan.railway_line_status AS railwayLineStatus 
                FROM
                    construction_control_plan AS constructionControlPlan
                    INNER JOIN station AS leftStation ON constructionControlPlan.start_station_id = leftStation.id
                    INNER JOIN station AS rightStation ON constructionControlPlan.end_station_id = rightStation.id
                    INNER JOIN railway_line AS railwayLine ON constructionControlPlan.railway_line_id = railwayLine.id 
                WHERE
                    1 = 1
    ...
    public Json dailyList() throws Exception {
        ...
        String sql = """
                    ...
                    FROM
                        ...
                        LEFT JOIN railway_line AS railwayLine ON constructionControlPlan.railway_line_id = railwayLine.id
                    WHERE
                        1 = 1
                """;
        ...
        sql += "ORDER BY constructionControlPlan.start_time\n";
