...
public class LkjTaskCustomController extends BaseController {
    ...
    public ModelAndView lkjDataLines() throws Exception {
        ...
        // 查询用户所在站段下的所有区间
        List<Organization> workshops = organizationClientService.getSlaves(user.getSectionId());
        List<Organization> workAreas = new ArrayList<>();
        workshops.forEach(x -> workAreas.addAll(organizationClientService.getSlaves(x.getId())));
    ...
    public ModelAndView insertPage() throws Exception {
        ...
        List<Station> stations = stationService.findStationsBySectionId(user.getSectionId());
