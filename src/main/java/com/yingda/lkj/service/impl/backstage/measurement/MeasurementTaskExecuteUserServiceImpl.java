...
public class MeasurementTaskExecuteUserServiceImpl implements MeasurementTaskExecuteUserService {
    ...
    @Override
    public void createByUserIds(String measurementTaskId, List<String> userIds) {
        // 先清空旧的执行人
        ...
        Map<String, User> userMap = StreamUtil.getMap(users, User::getId, x -> x);

        List<MeasurementTaskExecuteUser> measurementTaskExecuteUsers =
                userIds.stream().map(x -> new MeasurementTaskExecuteUser(measurementTaskId, userMap.get(x))).collect(Collectors.toList());
